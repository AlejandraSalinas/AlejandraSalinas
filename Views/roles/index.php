        ...
        <div class="row">
            <div class="col">
                <div class="card card-body">
                    <h3>Crear Rol:</h3>
                    <div class="mb-3">
                        <form action="../../controllers/RolesController.php?c=1" method="POST">
                            <div class="input-group ">
                                <div class="input-group mb-3">
                                    <input type="text" name="nombre" id="nombre" class="form-control" placeholder="Ingrese un nuevo Rol" required>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr>
                            ...
                            if ($registro_rol) {
                                $pos = 1;
                                foreach ($registro_rol as $rol) {

                            ?>
                        <tr>
                            <td><?= $pos ?></td>
                            <td>
                                <span id="rol<?= $rol->getId() ?>"> <?= $rol->getRoles() ?> </span>
                            </td>
                            <td>
                            <td>
                                <a class="btn btn-sm btn-outline-warning" onclick="show(<?= $rol-> getId()  ?>)">
                                    <i class="bi bi-pencil-square" style="font-size: 1.4rem;"></i>
                                </a>
                                <a class="btn btn-sm btn-outline-danger" href="../../controllers/RolesController.php?c=4&id_rol=<?= $rol-> getId() ?>">
...
<script>
    ...
    function show(id_rol) {
        ...
        let elemento = document.getElementById(`rol${id_rol}`);
        let documento = elemento.textContent

        document.getElementById('nombre').value = documento
        document.getElementById('nombre').setAttribute('data-id', id_rol);
    }

    function editar() {

        let elemento = document.getElementById("nombre");
        let id_rol = elemento.dataset.id
        let nombre = elemento.value

        axios.post(`../../controllers/RolesController.php?c=3&id_rol=${id_rol}&nombre=${nombre}`)
